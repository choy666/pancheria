...
- Cada sucursal puede configurar horarios de apertura (con soporte de turnos overnight, interpretados en `NEXT_PUBLIC_BRANCH_TIMEZONE`), dirección, ubicación, teléfonos con etiqueta y redes sociales (`branches.phones`, `branches.social_links`). Los teléfonos y redes se muestran en `/pedido`, en el diálogo de pedido creado y en el encabezado del chat.
- Con horarios configurados, el panel muestra avisos de caja por turno (`fuera_de_horario`, `cierre_recomendado`); sin horarios se usa el fallback por umbral `CAJA_OVERDUE_HOURS` (12 h por defecto).
...
- Al crear el pedido, el cliente puede ir al chat `/pedido/[id]/chat?token=...` para coordinar con la sucursal. El chat soporta texto, imágenes y ubicación.
  - El cliente con entrega a domicilio (`delivery`) puede compartir su ubicación actual desde el chat; el enlace se genera con el proveedor de mapas configurado.
  - El operador con retiro en sucursal (`pickup`) puede enviar la ubicación de la sucursal (`branch.location`) desde el panel a través de `POST /api/pedidos/[id]/chat/ubicacion`.
  - Los enlaces de mapas se detectan y renderizan como "Ver ubicación" en `ChatMessageList`, y `PedidoInfo` convierte la dirección del cliente en un enlace de búsqueda.
  - El proveedor de mapas y la URL base se configuran con `NEXT_PUBLIC_MAPS_PROVIDER` y `NEXT_PUBLIC_MAPS_BASE_URL` (ver `.env.example`).
...
`NEXT_PUBLIC_PEDIDO_REFETCH_INTERVAL_MS`, `NEXT_PUBLIC_CATALOG_PAGE_SIZE`, `NEXT_PUBLIC_PEDIDOS_REFRESH_INTERVAL_MS`, `NEXT_PUBLIC_DASHBOARD_REFRESH_INTERVAL_MS`, `NEXT_PUBLIC_API_TIMEOUT_MS`, `NEXT_PUBLIC_CHAT_REFRESH_INTERVAL_MS`, `NEXT_PUBLIC_CHAT_MAX_TEXT_LENGTH`, `NEXT_PUBLIC_CHAT_PAGE_SIZE`, `NEXT_PUBLIC_CHAT_IMAGE_MAX_SIZE_MB`, `NEXT_PUBLIC_CHAT_ALLOWED_IMAGE_MIME_TYPES`, `PUBLIC_CHAT_RATE_LIMIT_WINDOW_MS`, `PUBLIC_CHAT_RATE_LIMIT_MAX_REQUESTS`, `CHAT_BRANCH_LOCATION_RATE_LIMIT_WINDOW_MS`, `CHAT_BRANCH_LOCATION_RATE_LIMIT_MAX_REQUESTS`, `NEXT_PUBLIC_MAPS_PROVIDER`, `NEXT_PUBLIC_MAPS_BASE_URL`, `PUBLIC_ORDER_RATE_LIMIT_STORE_PROVIDER`, `PUBLIC_ORDER_RATE_LIMIT_WINDOW_MS`, `PUBLIC_ORDER_RATE_LIMIT_MAX_REQUESTS`, `PUBLIC_POLL_RATE_LIMIT_WINDOW_MS`, `PUBLIC_POLL_RATE_LIMIT_MAX_REQUESTS`, `PUBLIC_CATALOG_CACHE_S_MAXAGE`, `PUBLIC_CATALOG_CACHE_SWR`, `PUBLIC_BRANCH_STATUS_CACHE_S_MAXAGE`, `PUBLIC_BRANCH_STATUS_CACHE_SWR`, `PUBLIC_RATE_LIMIT_TRUST_PRIVATE_IPS`, `E2E_ENABLE_RATE_LIMIT`, `ORDER_EXPIRATION_MS`, `EXPIRE_ORDERS_TIME_BUDGET_MS`, `ORDER_MESSAGES_RETENTION_DAYS`, `TRASH_RESTORE_BATCH_SIZE`, `CAJA_SUMMARY_PAGE_SIZE`, `CRON_SECRET`, `TRUSTED_PROXY_IP_HEADER`, `LOCAL_STORAGE_PATH`, `CHAT_LOCAL_STORAGE_PATH`, `RATE_LIMIT_STORE_PROVIDER`, `LOGIN_ATTEMPTS_RETENTION_MS`, `DATABASE_POOL_MAX`, `DATABASE_CONNECTION_TIMEOUT_MS`, `DATABASE_IDLE_TIMEOUT_MS`, `NEXT_PUBLIC_CAJA_AUTO_CLOSE_HOURS`, `CAJA_DEFAULT_HISTORY_DAYS`, `CAJA_OVERDUE_HOURS`, `NEXT_PUBLIC_BRANCH_TIMEZONE`, `DEFAULT_BRANCH_NAME`, `DEFAULT_BRANCH_ADDRESS`, `DEFAULT_BRANCH_PHONE`, `DEFAULT_BRANCH_LOCATION`, `DEFAULT_BRANCH_SOCIAL_LINKS`, `NEW_BRANCH_NAME`, `NEW_BRANCH_USERNAME`, `NEW_BRANCH_PASSWORD`, `NEW_BRANCH_ADDRESS`, `NEW_BRANCH_PHONE`, `NEW_BRANCH_LOCATION`, `NEW_BRANCH_SOCIAL_LINKS`.
...
- `GET /api/cron/rate-limit-cleanup` — limpia entradas vencidas de `public_order_rate_limits` y purga `login_attempts` más viejos que `LOGIN_ATTEMPTS_RETENTION_MS` (por defecto 7 días).
