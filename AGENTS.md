...
>
> Para correr E2E de forma confiable se requiere una base de datos descartable, `ADMIN_USERNAME`/`ADMIN_PASSWORD` consistentes con el seed y que `AUTH_URL`/`NEXTAUTH_URL` apunten a `http://localhost:3000`. El `playwright.config.ts` carga `.env.e2e` como base y luego `.env.local` con prioridad, por lo que se puede usar `.env.local` directamente para las pruebas. Si se prefiere un archivo separado, copiar `.env.e2e.example` a `.env.e2e` y completar `DATABASE_URL` con una base descartable; el `global-setup.ts` valida que la URL sea local o tenga un sufijo de test/e2e/qa/staging y aborta si no lo es.
>
...
>
> El `webServer` de `playwright.config.ts` levanta `npm run dev:e2e`, que carga `.env.e2e` y luego `.env.local` con `dotenv` antes de iniciar Next.js, y espera a que `/api/caja/resumen` responda para forzar la compilación de una ruta API antes de iniciar los tests. También se puede levantar manualmente con `npm run dev:e2e` y correr `NO_WEB_SERVER=1 npx playwright test`; esto es necesario si `.env.local` apunta a producción o si se quiere reutilizar un servidor ya calentado.
