...
- Workaround del body en POST: bajo `next dev` con Turbopack (Next.js 16.3.0), las rutas `POST` de chat reciben `Content-Length` correcto pero `request.body === null`, por lo que `request.json()` arroja `SyntaxError: Unexpected end of JSON input`. Para mitigarlo, el cliente envía el campo `content` como query param (`?content=...`) y el handler fallbacka a leer el query si el body está vacío. Los handlers también aceptan JSON body, por lo que los tests unitarios no dependen del query param. Ver <ref_file file="C:/developer/paginas/pancheria/src/app/api/public/pedido/[id]/chat/route.ts" /> y <ref_file file="C:/developer/paginas/pancheria/src/app/api/pedidos/[id]/chat/route.ts" />. Mantener este workaround documentado hasta que Next.js corrija el bug de body nulo o se migre el POST a Pages Router.
