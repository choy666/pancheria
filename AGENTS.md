...
>
> El `webServer` de `playwright.config.ts` levanta `npm run dev:e2e`, que carga `.env.local` y luego `.env.e2e` con `dotenv` antes de iniciar Next.js, y espera a que `/api/caja/resumen` responda para forzar la compilación de una ruta API antes de iniciar los tests. También se puede levantar manualmente con `npm run dev:e2e` y correr `NO_WEB_SERVER=1 npx playwright test`; esto es necesario si `.env.local` apunta a producción o si se quiere reutilizar un servidor ya calentado.
