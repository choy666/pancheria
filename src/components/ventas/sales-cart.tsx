...
import { getProductAdditional, type SellableProduct } from '@/lib/ventas-helpers';
import {
  formatMoney,
  moneyToNumber,
  multiplyMoney,
...
import type { SellableCartLine } from '@/hooks/useSellableCart';
...
interface SalesCartProps {
  cart: SellableCartLine<SellableProduct>[];
...
export function SalesCart({
  ...
  const top = cartDisabled ? (
    ...
    >
      La caja está cerrada. Abrí la caja desde el panel superior para comenzar a
      vender.
    </div>
  ) : null;
  ...
  const footer = (
    <>
      <div className="border-t border-white/10 pt-4">
        <p className="font-mono text-2xl font-bold" aria-live="polite">
