...
function buildRequest(id: string): NextRequest {
  return new NextRequest(
    `http://localhost:3000/api/pedidos/${id}/finalizar`,
    { method: 'POST' }
  );
}

describe('POST /api/pedidos/[id]/finalizar', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    const session = {
  ...
  test('finaliza el pedido y devuelve 200', async () => {
    const order = { id: 1, status: 'finished' };
    mockedOrderService.finishOrder.mockResolvedValue(order as any);
    ...
    expect(mockedOrderService.finishOrder).toHaveBeenCalledWith({
      branchId: BRANCH_ID,
      orderId: 1,
    });
  });
  ...
  test('devuelve 404 cuando el pedido no existe', async () => {
    mockedOrderService.finishOrder.mockRejectedValue(
  ...
  test('devuelve 400 ante un ValidationError', async () => {
    mockedOrderService.finishOrder.mockRejectedValue(
      new ValidationError('Solo se puede finalizar un pedido pagado.')
    );
    ...
    expect(response.status).toBe(400);
    expect(body.error).toBe('Solo se puede finalizar un pedido pagado.');
  });
});
