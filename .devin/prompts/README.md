...
- [Diagnóstico del contador de horarios en `/sucursales`](archivados/diagnostico-columna-horarios-sucursales.md) — resuelto: la columna contaba franjas (`openingHours.length`) y las etiquetaba "días"; la sucursal ID 3 tiene 10 franjas válidas en 7 días únicos (dobles turnos contiguos). `branch-list.tsx` ahora muestra `{días únicos} días` y `· {franjas} franjas` solo cuando difieren; verificación completa en verde (lint, tsc, 1941 tests, knip, build, E2E sucursales 9/9).
- [Validación en vivo, preview de mapa y copiado de horarios en el formulario de sucursal](archivados/sucursal-form-validacion-mapa-y-horarios.md) — resuelto: `tryBuildLocationUrl` extrae el `src` del `<iframe>` pegado (bug de "Insertar un mapa"), `describeLocationInput` clasifica el input para feedback en vivo (error inline tras tocar el campo, hint de enlace, preview en `<details>` con `branch-location-preview.tsx`), `coordinatesFromMapUrl` extrae `/@lat,lng` de URLs de Google, y los horarios se validan en vivo (`branch-hours-error`) con botones Copiar/Pegar por día; E2E `sucursal-form-ux.spec.ts` + `sucursal-contactos-y-turnos.spec.ts`: 8 passed.
