...
- [Plan de auditoría QA integral (2026-09-21)](plan-auditoria-qa-2026-09-21.md) — Etapa 1 del prompt `auditoria-qa-integral.md`: baseline, matriz de riesgos, cobertura por capa y alcance propuesto (ejecutado; ver informe).
- [Auditoría QA integral (2026-09-21)](auditoria-qa-2026-09-21.md) — ejecución por capas: 3 hallazgos (expiración omitida en `receiveOrder`/`convertOrderToSale`, `idempotencyKey` por intento, upload no-multipart → 500), concurrencia y seguridad verificadas, axe WCAG 2.2 sin violaciones, WebKit y build prod OK; incluye lección de higiene de la suite E2E.
- [Auditoría QA ronda 2 (2026-09-23)](auditoria-qa-ronda-2-2026-09-23.md) — CSP contra `next start`: P0 corregido (`/pedido/seguimiento` estática sin nonce → no hidrataba en producción); `X-Forwarded-For` verificado fail-closed + fix menor de info-leak; revisión visual con daltonismo sin hallazgos.
