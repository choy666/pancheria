...
- **Un pedido público `pending` no reserva stock; la reserva se crea al recibirlo (`in_process`).** `createOrder` valida disponibilidad pero no reserva ni descuenta stock; `receiveOrder` inserta las reservas en `order_stock_reservations` con un movimiento `reserve` en `stock_movements` al pasar el pedido a `in_process`; `convertOrderToSale` y `cancelOrder` liberan la reserva solo si el pedido estaba `in_process`, y la expiración de `pending` no toca stock porque nunca reservó. Esto evita bloquear insumos en pedidos que el operador aún no confirmó.
- **La expiración de pedidos no debe depender del texto del error.** `expirePendingOrders` usa `cancelExpiredOrder`, que bloquea la fila con `findByIdForUpdate` y solo cancela si el pedido sigue `pending`; esto evita anular ventas si el pedido fue confirmado entre el listado y la cancelación. Se toleran `DomainError` genéricos y se devuelve la cantidad realmente expirada.
...
- **La superficie pública (`/api/public/*`) no debe transportar datos internos de stock.** Sanear en los wrappers públicos (`validatePublicCart`, `toPublicCatalogProduct`): `shortageByProduct` expone solo la presencia del faltante, y el `breakdown` por insumo no viaja al cliente. Para errores, preferir un `code` estructurado (`INSUFFICIENT_STOCK` con `productName`) y mensajes amigables en la ruta pública; el detalle técnico (`insumo`, cantidades) se conserva en `InsufficientStockError.message` para el panel. El cliente mapea con `toPublicErrorMessage` (`src/lib/public-errors.ts`), que usa el código y mantiene un regex como fallback.
...
- **Compartir ubicación en el chat sin migrar el esquema.** Para envíos a domicilio (`delivery`) se usa `navigator.geolocation` en el cliente y se envía un enlace de mapas como `content`; para retiro en sucursal (`pickup`) el operador envía `branch.location` desde `POST /api/pedidos/[id]/chat/ubicacion` con rate limit por `branchId`. El proveedor y la URL base se configuran con `NEXT_PUBLIC_MAPS_PROVIDER` y `NEXT_PUBLIC_MAPS_BASE_URL`; no hardcodear dominios de mapas. `navigator.geolocation` requiere un contexto seguro (`https://` o `localhost`).
- **Compartir ubicación por chat requiere validación en cliente y servidor.** El cliente usa `buildMapCoordinatesUrl` / `buildMapSearchUrl` (`src/lib/maps.ts`) y el input se sanitiza con `isValidLocationUrl` / `tryBuildLocationUrl` antes de enviarse. El endpoint `POST /api/pedidos/[id]/chat/ubicacion` vuelve a validar la URL, rechaza protocolos distintos de `http`/`https` y aplica rate limit por `branchId` (`CHAT_BRANCH_LOCATION_RATE_LIMIT_WINDOW_MS` y `CHAT_BRANCH_LOCATION_RATE_LIMIT_MAX_REQUESTS`). Los defaults de `NEXT_PUBLIC_MAPS_PROVIDER` (`openstreetmap`, `google`, `waze`) son plantillas de dominios públicos y pueden sobrescribirse con `NEXT_PUBLIC_MAPS_BASE_URL`; no contienen secretos.
