...
- **Un pedido público reserva stock inmediatamente al crearse.** `createOrder` valida disponibilidad, inserta la orden y reserva los insumos críticos en `order_stock_reservations` con un movimiento `reserve` en `stock_movements`; `convertOrderToSale` libera la reserva y descuenta al confirmar; `cancelOrder` y la expiración liberan la reserva. Esto evita la sobreventa entre pedidos `pending` y ventas.
