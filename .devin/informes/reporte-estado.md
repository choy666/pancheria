...
## 0. Estado vigente (auditoría 2026-09-24; PR #9)

**Baseline Git:** `main` en `090fa63ec958c57dcecd3fd527f3a7391307b7cd` (`docs: cerrar estado post-merge — PR #8, CI de main verde, ramas limpias`). **Estado al cierre:** la implementación QA-04, la migración generada, pruebas y correcciones documentales quedaron commiteadas en `fix/qa04-payload-fingerprint` y publicadas en el PR #9. El commit funcional principal es `61f3206bfa91a4fada2cf18fa5e5481569b9fac1`; los commits documentales posteriores solo actualizan este informe. `main` sigue en `090fa63`.

### Baseline Git y merges

- Al inicio, `git status --short --branch` mostró `main...origin/main` sin cambios trackeados y `.vscode/extensions.json` sin trackear. Ese archivo quedó intacto y fuera del commit.
- El commit funcional de la rama es `61f3206bfa91a4fada2cf18fa5e5481569b9fac1`; `fix/qa04-payload-fingerprint` también contiene actualizaciones documentales posteriores de este informe. `main` sigue alineada con `origin/main` en `090fa63`.
- `git branch -a`: baseline inicial con `main` y `origin/main`; después se creó y publicó `origin/fix/qa04-payload-fingerprint`.
- `gh pr list --state open`: no había PRs en la consulta de baseline. Luego se creó el **PR #9** (`fix: rechazar reutilización divergente de claves de idempotencia`), abierto con `mergeStateStatus: CLEAN` tras el CI verde.
...
El run `35948485239` valida remotamente la implementación del PR #9 sobre `61f3206`; el run `35949717967` repitió el CI completo sobre `ae81663`, que solo agregaba esta actualización documental. Los runs fallidos/cancelados son anteriores al CI verde del HEAD base. `.github/workflows/ci.yml` mantiene serialización por shard (`cancel-in-progress: false`), sharding opt-in y default `[1]`; el nuevo `npx drizzle-kit check` ya se ejecutó en Actions y terminó `success` antes de `migrate`.
...
- `npx drizzle-kit check` pasó localmente (`Everything's fine`) y en el run `35948485239` del PR #9: historial de migraciones consistente, sin modificar ninguna base en el check; el paso siguiente aplicó `0032` a la base E2E descartable del CI.

### Estado funcional verificado por módulo

“Verificado” aquí significa que existen las piezas de código y tests indicados, que los checks locales pasaron y que el PR #9 obtuvo CI remoto verde sobre `61f3206` y `ae81663`. Esto no equivale a un smoke test de producción ni a hardware externo.
...
| QA-04: misma `idempotencyKey` con payload distinto | **menor** | **Implementado en PR #9 y validado por CI remoto sobre `61f3206` y `ae81663`; `0032` aplicada a `.env.e2e` y a la base E2E del CI.** Las pruebas unitarias cubren pedidos, ventas directas, conversiones, carreras, legacy y HTTP 409. Tras actualizar las specs, la última E2E local pasó 175/175 y los E2E remotos del PR también pasaron. Falta mergear el PR y aplicar `0032` al entorno destino autorizado. | `src/application/idempotencyService.ts`, `src/application/services/orderService.ts`, `src/application/services/saleService.ts`, `src/db/schema.ts`, `drizzle/0032_icy_shaman.sql`, runs `35948485239`/`35949717967` y tests asociados. |
...
| Neon efímera por `run_id` / sharding E2E | **menor** | **Parcial.** PR #7 serializó por shard y elevó el timeout a 25 min. No hay aprovisionamiento efímero por `run_id`; `E2E_SHARDS` sigue opt-in y default `[1]`. El último E2E remoto del HEAD base tomó 18 min 06 s y los del PR #9 tomaron 12 min 26 s y 15 min 05 s: todos superan el umbral documentado de ~10 min. | `.github/workflows/ci.yml`; `archivados/ci-e2e-base-compartida.md`; runs `35878657216`, `35948485239` y `35949717967`. |
...
| Validación de consistencia de migraciones en CI | **menor** | **Implementada en PR #9 y verificada en Actions.** `npx drizzle-kit check` precede a `drizzle-kit migrate` en el job E2E; ambos pasos terminaron `success` en los runs `35948485239` y `35949717967`. Queda mergear el PR. | `.github/workflows/ci.yml`; runs `35948485239`/`35949717967`; documentación oficial de `drizzle-kit check`; `archivados/auditoria-deploy-vercel-2026-09-14.md` §Conclusión. |
...
- La auditoría de deploy recomendaba verificar consistencia de migraciones en CI. `migrate` en la base E2E valida la aplicación, pero no comprueba el historial generado; por eso agregué `npx drizzle-kit check` antes de `migrate` en el job E2E y documenté el comando en `AGENTS.md`, README y checklist. Los runs `35948485239` y `35949717967` ejecutaron ambos pasos en verde sobre el PR #9.
...
| P1 | Alto × medio | Revisar y mergear el PR #9, luego aplicar `0032` solo al entorno destino autorizado siguiendo el proceso de migraciones. El CI remoto ya validó la implementación sobre `61f3206` y el HEAD documental `ae81663`; la migración de producción/staging sigue sin ejecutarse. |
...
| P2 | Medio × medio | Planificar sharding E2E con una base descartable independiente por shard: el último run base tomó 18 min 06 s y los del PR #9 tomaron 12 min 26 s y 15 min 05 s, todos por encima del umbral orientativo de 10 min. No activar `E2E_SHARDS` hasta aprovisionar y verificar cada par de secrets/base; Neon efímera por `run_id` es una fase posterior. |
| P3 | Bajo × bajo | Después del merge, revisar `VERCEL_PRODUCTION_URL`, métricas Neon/Vercel y DRS sin exponer valores; hacer smoke test de producción tras aplicar la migración autorizada. |
...
2. Las specs E2E con expectativas antiguas ya se actualizaron; la última corrida local pasó 175/175 y el E2E remoto del PR #9 pasó. Toda corrida futura sobre `.env.e2e` requiere autorización porque `global-setup.ts` vuelve a truncar y reseedear. `0032` ya quedó aplicada allí.
3. Revisar y mergear el PR #9 si se aprueba el cambio; los runs `35948485239` y `35949717967` ya validaron `drizzle-kit check`, migración E2E, unitarios, build, E2E/accesibilidad y reporte consolidado sobre `61f3206` y `ae81663`.
...
- Los runs remotos `35948485239` y `35949717967` del PR #9 terminaron `success` sobre `61f3206` y `ae81663`, incluidos `drizzle-kit check`, `migrate` sobre base E2E descartable, unitarios, build, E2E/accesibilidad y reporte consolidado. No se hizo merge ni deploy; cualquier commit posterior debe revisarse en el estado final del PR antes de mergear.
