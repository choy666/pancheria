...
- Los teléfonos y redes se exponen públicamente en `/pedido` (tarjeta de sucursal en el encabezado del catálogo), en el diálogo de pedido creado y en el encabezado del chat. El seed los configura con `DEFAULT_BRANCH_PHONE`/`DEFAULT_BRANCH_SOCIAL_LINKS` y `NEW_BRANCH_PHONE`/`NEW_BRANCH_SOCIAL_LINKS`.
- La `location` se muestra como mapa embebido (`<iframe>` dentro de un "Ver mapa" colapsable que solo se carga al abrirlo) cuando el valor lo permite: coordenadas `lat,lng`, URL del proveedor con marcador (`mlat`/`mlon`, `#map=`, `/@lat,lng` en el path, `query=lat,lng`) o URL ya embebible (`/export/embed.html`, `/maps/embed`, `output=embed`). Los short links (`maps.app.goo.gl`, `goo.gl/maps`), Waze y otros orígenes se muestran solo como enlace "Ver en mapa". Los orígenes del iframe salen de `getMapsFrameOrigins` (`src/config/maps.ts`) y habilitan `frame-src` en la CSP; `buildMapEmbedUrl` (`src/lib/maps.ts`) valida contra esa misma lista.
- El formulario de sucursal acepta además el HTML completo del `<iframe>` de "Insertar un mapa" (se extrae el `src` y se valida como URL http(s); el HTML nunca se renderiza), valida la ubicación en vivo con `describeLocationInput` (`src/lib/maps.ts`) y ofrece una preview del mapa (`branch-location-preview.tsx`) cuando es embebible. Los horarios también se validan en vivo y pueden copiarse de un día a otro con los botones "Copiar"/"Pegar".
